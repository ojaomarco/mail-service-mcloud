...
class Sender():
    ...
    @staticmethod
    def _email_builder_active( user, serial_number, status, is_running, info_data, issues):
        try:
    ...
    def _email_builder_deactivated( user, serial_number, status, is_running):
        # html_email = HTML_TEMPLATE.format(user_name=user, status=status, serial_number=serial_number, is_running=is_running)
        return 'html_email'
