...
class Processer:
    ...
    def process_issues(self, issues):
        try:
            ...
            result["opened_at"] = pd.to_datetime(result["opened_at"]).dt.strftime(
                "%d/%m %H:%M"
            )
            result["closed_at"] = pd.to_datetime(result["closed_at"]).dt.strftime(
                "%d/%m %H:%M"
            )
            result = result[["opened_at", "closed_at", "name"]]
            result["closed_at"] = result["closed_at"].fillna(result["opened_at"])
